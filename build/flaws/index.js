...
async function fixFixableFlaws(doc, options, document) {
  ...
  if (isMarkdown) {
    ...
    if (options.fixFlawsTypes.has("bad_pre_tags")) {
    ...
    if (options.fixFlawsTypes.has("images")) {
      ...
      for (const flaw of doc.flaws.images || []) {
        ...
        if (flaw.suggestion) {
          ...
          try {
            const decodedPathname = decodeURI(url.pathname).replace(
              /\s+/g,
              "_"
            );
    ...
    if (options.fixFlawsTypes.has("image_widths")) {
