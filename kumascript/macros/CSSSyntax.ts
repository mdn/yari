...
import type { KumaThis } from "../src/environment";
 ...
 */
export async function render(this: KumaThis, $0) {
  const { env, mdn } = this;
  ...
  for (const spec of Object.values(parsedWebRef)) {
    valuespaces = { ...valuespaces, ...(spec as any).valuespaces };
  }
